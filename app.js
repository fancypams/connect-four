        ...
        (function(index) {
            squares[i].onclick = function() {
                if (squares[index + 7].classList.contains('taken')) {
                    ...
                    if (currentPlayer === 'Liz') {
                        squares[index].classList.add('taken')
                        squares[index].classList.add('player-one')
                        currentPlayer = 'Jenna'
                        displayCurrentPlayer.innerHTML = currentPlayer
                    } else if (currentPlayer === 'Jenna') {
                        squares[index].classList.add('taken')
                        squares[index].classList.add('player-two')
                        currentPlayer = 'Liz'
                        displayCurrentPlayer.innerHTML = currentPlayer
                    }
                } else alert('Try again, Dummy!')
            }
        })(i)
    }
    ...
    function checkBoard() {
        ...
        for (let y = 0; y < winningArrays.length; y++) {
            ...
            if (square1.classList.contains('player-one') &&
                ...
                result.innerHTML = 'Liz Wins!'
                document.querySelector('#turn').style.display = "none"
                document.querySelector('#result').style.display = "block"
                result.classList.add('liz')
            } else if (square1.classList.contains('player-two') &&
                square2.classList.contains('player-two') &&
                square3.classList.contains('player-two') &&
                square4.classList.contains('player-two')) {
                result.innerHTML = 'Jenna Wins!'
                document.querySelector('#turn').style.display = "none"
                document.querySelector('#result').style.display = "block"
                result.classList.add('jenna')
            }
        }
    }
